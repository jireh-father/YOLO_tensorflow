...
class YOLO_TF:
    ...
    def __init__(self, argvs=[]):
        ...
        elif self.fromfiles is not None:
            self.detect_from_files(self.fromfiles)

    def argv_parser(self, argvs):
        for i in range(1, len(argvs), 2):
            if argvs[i] == '-fromfile': self.fromfile = argvs[i + 1]
            if argvs[i] == '-fromfiles': self.fromfiles = argvs[i + 1].split(",")
    ...
    def detect_from_cvmats(self, img_list):
        inputs = np.zeros((len(img_list), 448, 448, 3), dtype='float32')
        ...
        in_dict = {self.x: inputs}
        net_output = self.sess.run(self.fc_19, feed_dict=in_dict)
        tofile_img = self.tofile_img
        tofile_txt = self.tofile_txt
        for i, img in enumerate(img_list):
            s = time.time()
            self.h_img, self.w_img, _ = img.shape
            self.result = self.interpret_output(net_output[i])
            if self.filewrite_img:
                filename_info = os.path.splitext(self.fromfiles[i])
                self.tofile_img = "%s_%s%s" % (filename_info[0], tofile_img, filename_info[1])
            if self.filewrite_txt:
                filename_info = os.path.splitext(self.fromfiles[i])
                self.tofile_txt = "%s_%s%s" % (filename_info[0], tofile_txt, ".txt")
            self.show_results(img, self.result)
            strtime = str(time.time() - s)
            if self.disp_console: print('Elapsed time : ' + strtime + ' secs' + '\n')

    def is_persons_from_cvmats(self, img_list):
        inputs = np.zeros((len(img_list), 448, 448, 3), dtype='float32')
        for i, img in enumerate(img_list):
            img_resized = cv2.resize(img, (448, 448))
            img_RGB = cv2.cvtColor(img_resized, cv2.COLOR_BGR2RGB)
        ...
        for i, img in enumerate(img_list):
            self.result = self.interpret_output(net_output[i])
            results.append(self.is_person_result(self.result))
        return results

    def is_person_result(self, results):
    ...
    def detect_from_files(self, filename_list):
    ...
    def interpret_output(self, output):
        ...
        offset = np.transpose(np.reshape(np.array([np.arange(self.grid_size)] * (2 * self.grid_size)),
                                         (2, self.grid_size, self.grid_size)), (1, 2, 0))
